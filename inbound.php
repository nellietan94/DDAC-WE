...
<html>
<head>
	<title>Maersk Line - West Europe - Inbound</title>
	<meta charset="utf-8">
...
<body>
	...
	<div class="container-fluid">
		...
		<a href="book_inbound.php"><button type="button" class="btn"><span class="glyphicon glyphicon-calendar"></span> BOOK</button></a>
		...
		<table class="table table-bordered">
			...
			<?php 
				...
				$query = "SELECT * FROM inbound ORDER BY id DESC";
				...
				foreach($result as $row){
					...
					if($row['status'] != 'CONFIRMED' && $row['status'] != 'CANCELLED'){
						echo '<a href="confirm_inbound.php?inbound='.$row['id'].'" onclick="confirm(\'Confirm Inbound?\')"><button class="btn btn-info margin-bottom-10">Confirm</button></a><br>';
						echo '<a href="cancel_inbound.php?inbound='.$row['id'].'" onclick="confirm(\'Cancel Inbound?\')"><button class="btn btn-danger">Cancel</button></a>';
					}else{
